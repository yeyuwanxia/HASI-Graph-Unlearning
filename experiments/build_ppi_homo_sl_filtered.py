...
CANONICAL_NAME = "ppi-homo-sl-filtered"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build PPI-Homo-SL-Filtered from PyG PPI raw files.")
    parser.add_argument("--raw_dir", default=str(ROOT / "data" / "raw" / "Planetoid" / "PPI" / "raw"))
    parser.add_argument(
        "--output_dir",
        default=str(ROOT / "data" / "raw" / "Planetoid" / "PPI" / "processed_ppi_homo_sl_filtered"),
    )
    parser.add_argument("--min_projected_class_count", type=int, default=50)
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit("torch-geometric is required to build PPI-Homo-SL-Filtered.") from exc
    ...
    original_label_freq = y_multi_all[nonzero_label_mask].sum(axis=0)
    ...
    for node in nonzero_indices:
        ...
        chosen_pos = positives[np.argmin(original_label_freq[positives])]
        projected_original[node] = int(chosen_pos)

    projected_counts = np.bincount(
        projected_original[projected_original >= 0],
        minlength=y_multi_all.shape[1],
    )
    retained_original_labels = np.flatnonzero(projected_counts >= int(args.min_projected_class_count))
    ...
    label_projection = {
        "rule": "rarest_positive",
        "tie_break": "lowest_original_label_id",
        "original_label_dim": int(y_multi_all.shape[1]),
        "original_label_frequencies": [int(value) for value in original_label_freq.tolist()],
        "projected_class_counts_before_filter": {
            str(idx): int(count) for idx, count in enumerate(projected_counts.tolist()) if int(count) > 0
        },
    ...
    class_counts = {
        "min_projected_class_count": int(args.min_projected_class_count),
    ...
    metadata = {
        ...
        "display_name": "PPI-Homo-SL",
        "source": "PyG PPI raw files",
        "construction": "disjoint_union_all_graphs",
        "task": "single-label node classification",
        "label_projection": "rarest_positive",
        "min_projected_class_count": int(args.min_projected_class_count),
