...
CANONICAL_NAME = "ppi-inductive-sl-filtered"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build PPI-Inductive-SL-Filtered from PyG PPI raw files.")
    parser.add_argument("--raw_dir", default=str(ROOT / "data" / "raw" / "Planetoid" / "PPI" / "raw"))
    parser.add_argument(
        "--output_dir",
        default=str(ROOT / "data" / "raw" / "Planetoid" / "PPI" / "processed_ppi_inductive_sl_filtered"),
    )
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit("torch-geometric is required to build PPI-Inductive-SL-Filtered.") from exc
    ...
    for node in nonzero_indices:
        ...
        chosen_pos = positives[np.argmin(original_label_freq_train[positives])]
