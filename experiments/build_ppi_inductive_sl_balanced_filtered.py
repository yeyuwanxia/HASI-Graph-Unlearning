...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build balanced single-label PPI from PyG PPI raw files.")
    parser.add_argument("--raw_dir", default=str(ROOT / "data" / "raw" / "Planetoid" / "PPI" / "raw"))
    parser.add_argument("--output_dir", default=None)
    parser.add_argument("--target_classes", type=int, default=20)
    parser.add_argument("--min_train_count", type=int, default=300)
    parser.add_argument("--max_train_share", type=float, default=0.30)
    parser.add_argument("--projection", default="balanced", choices=["balanced"])
...
def main() -> None:
    args = parse_args()
    if int(args.target_classes) <= 0:
        raise SystemExit("--target_classes must be positive.")
    if int(args.min_train_count) < 0:
        raise SystemExit("--min_train_count must be non-negative.")
    if not 0 < float(args.max_train_share) <= 1:
        raise SystemExit("--max_train_share must be in (0, 1].")

    target_classes = int(args.target_classes)
    dataset_name = f"ppi-inductive-sl-balanced{target_classes}-filtered"
    default_output_dir = (
        ROOT
        / "data"
        / "raw"
        / "Planetoid"
        / "PPI"
        / f"processed_ppi_inductive_sl_balanced{target_classes}_filtered"
    )
    ...
    try:
        ...
        raise SystemExit("torch-geometric is required to build balanced PPI single-label data.") from exc
    ...
    original_label_freq_train = y_multi_all[train_source_mask].sum(axis=0)
    train_node_count = int(train_source_mask.sum())
    max_train_count = float(args.max_train_share) * float(train_node_count)

    eligible_labels = [
        int(label)
        for label, count in enumerate(original_label_freq_train.tolist())
        if int(count) >= int(args.min_train_count) and float(count) <= max_train_count
    ]
    eligible_labels = sorted(eligible_labels, key=lambda label: (-int(original_label_freq_train[label]), label))
    if len(eligible_labels) < target_classes:
        raise SystemExit(
            "Not enough eligible labels for balanced projection: "
            f"eligible={len(eligible_labels)}, target_classes={target_classes}, "
            f"min_train_count={args.min_train_count}, max_train_share={args.max_train_share}."
        )

    retained_original_labels = np.array(eligible_labels[:target_classes], dtype=np.int64)
    retained_label_set = set(int(label) for label in retained_original_labels.tolist())
    retained_label_mask = np.zeros(y_multi_all.shape[1], dtype=bool)
    retained_label_mask[retained_original_labels] = True

    candidate_matrix = (y_multi_all > 0) & retained_label_mask
    final_node_mask = candidate_matrix.any(axis=1)
    projected_original = np.full(y_multi_all.shape[0], -1, dtype=np.int64)
    assignment_counts_by_split: dict[str, dict[str, int]] = {}

    for split in SPLITS:
        assigned_counts = {int(label): 0 for label in retained_original_labels.tolist()}
        split_indices = np.flatnonzero((split_name_all == split) & final_node_mask)
        for node in split_indices.tolist():
            candidates = [int(label) for label in np.flatnonzero(candidate_matrix[node])]
            chosen_label = min(
                candidates,
                key=lambda label: (
                    assigned_counts[label],
                    -int(original_label_freq_train[label]),
                    label,
                ),
            )
            projected_original[node] = chosen_label
            assigned_counts[chosen_label] += 1
        assignment_counts_by_split[split] = {
            str(label): int(assigned_counts[int(label)])
            for label in retained_original_labels.tolist()
        }
    ...
    selected_train_counts = {
        str(label): int(original_label_freq_train[int(label)])
        for label in retained_original_labels.tolist()
    }
    label_projection = {
        "rule": "balanced_topk_positive",
        "projection": str(args.projection),
        "fit_scope": "official_train_graphs",
        "selection_rule": "top_train_frequency_after_min_count_and_max_share_filter",
        "assignment_scope": "per_official_split",
        "assignment_rule": "least_assigned_candidate_label",
        "tie_break": "higher_train_frequency_then_lower_original_label_id",
        "target_classes": int(target_classes),
        "min_train_count": int(args.min_train_count),
        "max_train_share": float(args.max_train_share),
        "max_train_count": float(max_train_count),
        ...
        "eligible_original_labels": [int(label) for label in eligible_labels],
        "selected_original_label_train_counts": selected_train_counts,
        ...
        "assignment_counts_by_split_original_label": assignment_counts_by_split,
    }
    class_counts = {
        "target_classes": int(target_classes),
        "min_train_count": int(args.min_train_count),
        ...
        "majority_baseline": {
            "train": _majority(train_counts),
            "val": _majority(val_counts),
            "test": _majority(test_counts),
        },
        "classes": [
            {
    ...
    metadata = {
        ...
        "display_name": f"PPI-Inductive-SL-Balanced{target_classes}",
        ...
        "label_projection": "balanced_topk_positive",
        "label_projection_fit_scope": "official_train_graphs",
        "projection_assignment_scope": "per_official_split",
        "target_classes": int(target_classes),
        "min_train_count": int(args.min_train_count),
        "max_train_share": float(args.max_train_share),
        ...
        "removed_no_selected_label_nodes": int((nonzero_label_mask & ~final_node_mask).sum()),
        ...
        "majority_baseline": class_counts["majority_baseline"],
...
def _majority(counts: np.ndarray) -> float:
