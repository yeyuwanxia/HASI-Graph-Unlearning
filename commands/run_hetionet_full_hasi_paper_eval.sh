...
RESULT_ROOT="${RESULT_ROOT:-results/mia_v2_hetionet-full-nosource_eval/hasi}"
HUB_SCORE_CACHE_ROOT="${HUB_SCORE_CACHE_ROOT:-results/mia_v2_hetionet-full-nosource_eval/hasi/artifacts/hub_scores}"
EXACT_RETRAIN_REFERENCE_ROOT="${EXACT_RETRAIN_REFERENCE_ROOT:-results/mia_v2_hetionet-full-nosource_eval/baselines/retrain/edge/artifacts/exact_retrain}"
...
validate_forget_set() {
  ...
  jq -e \
    ...
    --argjson forget_seed "$forget_seed" '
      .dataset == "hetionet-full-nosource" and
...
for kind in "${kinds[@]}"; do
  ...
  for variant in default; do
    if [[ "$variant" == "default" ]]; then
      ...
      config="configs/tuned/by_dataset/hetionet-full-nosource/${kind}.yaml"
      method=hasi_tuned
    fi
    ...
    for ratio in r0p05 r0p1; do
      for spec in "42 70042" "123 70123" "2024 72024"; do
        ...
        base_path="results/shared_base/hetionet-full-nosource/seed${base_seed}"
        forget_path="experiments/forget_sets_eval/hetionet-full-nosource/hetionet-full-nosource_${kind}_${ratio}_${selection}_base${base_seed}_fseed${forget_seed}.json"
        output_dir="$RESULT_ROOT/$method/$kind"
        output="$output_dir/${method}_hetionet-full-nosource_${kind}_${ratio}_${selection}_base${base_seed}_fseed${forget_seed}.json"
        reference_path=""
        extra_args=()
        if [[ "$kind" == "edge" ]]; then
