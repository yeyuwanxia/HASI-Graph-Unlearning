...
RESULT_ROOT="${RESULT_ROOT:-results/mia_v2_hetionet-full-nosource_eval/baselines}"
...
aggregate_results() {
  ...
  run_command "${PY[@]}" experiments/aggregate_results.py \
    ...
    --pattern '*hetionet-full-nosource_*.json' \
  ...
  for baseline in "${baselines[@]}"; do
    ...
    for ratio in r0p05 r0p1; do
      ...
      for spec in "42 70042" "123 70123" "2024 72024"; do
        ...
        output="$output_dir/${method_key}_hetionet-full-nosource_${kind}_${ratio}_${selection}_base${base_seed}_fseed${forget_seed}.json"
        reference_path=""
        if [[ "$kind" == "edge" ]]; then
